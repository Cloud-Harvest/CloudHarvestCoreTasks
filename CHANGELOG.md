...
- `HarvestUpdateTask` now includes the following metadata fields. 
  - `TemplateIdentifier`: the identifier of the template used to create the task chain.
  - `SingletonKeys` / `singleton_keys` (directive): the keys necessary to collect the record as a singleton instead of with all other records. Defaults to the `UniqueIdentifierKeys`.
  - `Singleton`: a dictionary representing the keys and values from `SingletonKeys` generated by the task.
  - Improved the way metadata field validation is performed.
  - Improved the performance of metadata updates by creating no-op logic paths.
