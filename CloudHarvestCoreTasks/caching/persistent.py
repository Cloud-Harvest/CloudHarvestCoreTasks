...
def set_pstar(**kwargs) -> ObjectId:
    ...
    Region. This function writes a PSTAR record to the persistent cache.
...
def prepare_record(record: dict, meta_extra_fields: tuple = ()) -> tuple:
    """
    Prepares a record to be written to the persistent cache. To qualify as a valid record, it must contain a key named
...
def write_records(records: list) -> list:
    """
    Writes a list of records to the persistent cache.
